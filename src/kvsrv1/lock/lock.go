...
func MakeLock(ck kvtest.IKVClerk, l string) *Lock {
	lk := &Lock{
		ck: ck,
		l: l,
		me: kvtest.RandValue(8), // 用来标识哪个客户端持有锁，正常情况下不应当用随机数，而是由客户端传入指定的 value
	}
	// You may add code here
	return lk
}
...
func (lk *Lock) Acquire() {
	...
	for {
		...
		if value != unlocked {
			log.Printf("Lab2 %v 发现分布式锁已被抢占, 循环尝试\n", lk.me)
		...
		log.Printf("Lab2 %v 尝试抢占分布式锁\n", lk.me)
		err2 := lk.ck.Put(lk.l, lk.me, version)

		// 如果成功持有锁，结束方法
		if err2 == rpc.OK {
			log.Printf("Lab2 %v 抢占分布式锁成功\n", lk.me)
			break
		}

		// 如果未成功抢占锁，循环尝试
		if err2 == rpc.ErrVersion {
			log.Printf("Lab2 %v 抢占分布式锁失败, 循环尝试\n", lk.me)
			continue
		}
		...
		if err2 == rpc.ErrMaybe {
			...
			if err != rpc.OK {
				log.Fatalf("Lab2 出现未预料的情况: Get 操作执行失败\n")
			}
			if value == lk.me {
				log.Printf("Lab2 ErrMaybe 情况下, Put RPC 命令实际执行成功, %v 抢占分布式锁成功\n", lk.me)
				break
			} else {
				log.Printf("Lab2 ErrMaybe 情况下, Put RPC 命令实际执行失败, %v 抢占分布式锁失败, 循环尝试\n", lk.me)
				continue
			}
		}
	}
...
func (lk *Lock) Release() {
	...
	for {
		value, version, err := lk.ck.Get(lk.l)
		if value != lk.me {
			log.Fatalf("Lab2 出现未预料的情况: 分布式锁未被当前进程 %v 持有, 而是 %v\n", lk.me, value)
		}
		if err != rpc.OK {
			log.Fatalf("Lab2 出现未预料的情况: Get 操作执行失败\n")
		}
		// 尝试释放锁
		log.Printf("Lab2 %v 尝试释放分布式锁\n", lk.me)
		err2 := lk.ck.Put(lk.l, unlocked, version)
		// 分布式锁释放成功
		if err2 == rpc.OK {
			log.Printf("Lab2 %v 释放分布式锁成功\n", lk.me)
			break
		}
		// 释放锁时，出现未预料的情况
		if err2 == rpc.ErrVersion {
			log.Fatalf("Lab2 出现未预料的情况: 串行的 Release 中调用 Put 发生 ErrVersion")
		}
		...
		if err2 == rpc.ErrMaybe {
			...
			if value == lk.me {
				log.Printf("Lab2 ErrMaybe 情况下, Put RPC 命令实际执行失败, %v 释放分布式锁失败, 循环重试\n", lk.me)
				continue
			} else {
				log.Printf("Lab2 ErrMaybe 情况下, Put RPC 命令实际执行成功, %v 释放分布式锁成功\n", lk.me)
				break
			}
		}
	}
