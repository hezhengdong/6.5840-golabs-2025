                ...
                while completed < total:
                    ...
                    for future in done:
                        ...
                        if timing:
                            ...
                            user_time = system_time = real_time = None
                            for i in range(len(words)):
                                word = words[i]
                                if word == 'user':
                                    user_time = words[i - 1]
                                elif word == 'sys':
                                    system_time = words[i - 1]
                                elif word == 'real':
                                    real_time = words[i - 1]
                            if user_time is not None and system_time is not None and real_time is not None:
                                try:
                                    results[test]['real_time'].add(float(real_time))
                                    results[test]['user_time'].add(float(user_time))
                                    results[test]['system_time'].add(float(system_time))
                                except ValueError as e:
                                    print(f"转换错误: {e}")
                            else:
                                print(f"未找到完整的时间信息: {line}")
